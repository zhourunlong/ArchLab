...
void NextPCGen::GenerateNextPC(mem_addr pc_addr, reg_word extended_imm, mem_addr target, bool branch, bool jump, ncycle_t current_cycle, bool direct_jump) {
    mem_addr next_pc = -1;
    if (direct_jump)
        next_pc = target;
    else {
        if (branch)
            next_pc = pc_addr + (extended_imm << 2);
        else
    ...
    //fprintf(stderr, "next PC = %08x, dont_gen = %d\n", next_pc, dont_gen);
